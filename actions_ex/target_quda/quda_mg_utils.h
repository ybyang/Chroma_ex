		...
		MGSubspacePointers* create_subspace(const SysSolverQUDAMULTIGRIDCloverParams& invParam)
		{
			...
			if( invParam.MULTIGRIDParamsP ) {
				...
				if( ip.verbosity == true ) {
					mg_inv_param.verbosity = QUDA_DEBUG_VERBOSE;
                                        mg_inv_param.verbosity_precondition = QUDA_DEBUG_VERBOSE;
				}
				else {
...
//                        mg_param.pre_orthonormalize = QUDA_BOOLEAN_YES;

                        for(int i=0; i<mg_param.n_level  ; i++)
                        if( invParam.MULTIGRIDParams->verbosity == true ) 
                        {
                             mg_param.verbosity[i] = QUDA_DEBUG_VERBOSE;
                        }
                        else
                        {
