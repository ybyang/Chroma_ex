...
namespace Chroma
{
  namespace LinOpSysSolverQUDAClover2Env
  {
    ...
    {
      //! Name to be used
      const std::string name("QUDA_CLOVER_INVERTER_v2");
    ...
    {
      return new LinOpSysSolverQUDAClover2(A, state,SysSolverQUDACloverParams(xml_in, path));
    }
...
#ifdef _PDF_
  LinOpSysSolverQUDAClover2::qudaInvert(const CloverTermT<T, U>::Type_t& clover,
				       const CloverTermT<T, U>::Type_t& invclov,
				       const T& chi_s,
				       T& psi_s) const{
#else				       
  LinOpSysSolverQUDAClover2::qudaInvert(const CloverTermT<T, U>& clover,
