...
#ifndef __syssolver_mdagm_quda_multigrid_clover_h_v2__
#define __syssolver_mdagm_quda_multigrid_clover_h_v2__
...
//#include <util_quda.h>
#ifdef BUILD_QUDA
#include "actions_ex/target_quda/quda_mg_utils.h"
#include "actions_ex/target_quda/mg_solver_exception.h"
#include <quda.h>
...
class MdagMSysSolverQUDAMULTIGRIDClover2 : public MdagMSystemSolver<LatticeFermion>
{
public:
	...
	MdagMSysSolverQUDAMULTIGRIDClover2(Handle< LinearOperator<T> > A_,
...
QDPIO::cout << solver_string << " init_time = "
	...
	~MdagMSysSolverQUDAMULTIGRIDClover2()
	{
		...
		if ( solution_good ) {
			...
			// Check solution
			{
				...
				if ( toBool( res_tmp.resid/sqrt(norm2(chi)) > invParam.RsdToleranceFactor * invParam.RsdTarget ) ) {
					...
					MGSolverException2 convergence_fail(invParam.CloverParams.Mass,
							invParam.SaveSubspaceID,
							res_tmp.n_count,
