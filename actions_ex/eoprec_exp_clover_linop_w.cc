...
{ 
 ...
 //! Creation routine with Anisotropy
				      ...
				      const CloverFermActParams& param_)
  {
    ...
    clov.create(fs, param,true);
 
    invclov.create(fs,param,clov,true);  // make a copy
					     ...
					     enum PlusMinus isign) const
  {
    ...
    swatch.reset(); swatch.start();
    clov.derivExp(ds_u, chi, psi, isign, 0);
    swatch.stop();
					       ...
					       enum PlusMinus isign) const
  {
    ...
    swatch.reset(); swatch.start();
    clov.derivExpMultipole(ds_u, chi, psi, isign, 0);
    swatch.stop();
    clov_deriv_time  += swatch.getTimeInSeconds();
						   ...
						   enum PlusMinus isign) const
  {
    ...
    invclov.derivExpTrLn(ds_u, isign, 0);
  ...
  {   
    ...
    swatch.reset(); swatch.start();
    clov.derivExp(ds_u, chi, psi, isign, 1);
    swatch.stop();
    clov_deriv_time += swatch.getTimeInSeconds();
					       ...
					       enum PlusMinus isign) const
  {
    ...
    swatch.reset(); swatch.start();
    clov.derivExpMultipole(ds_u, chi, psi, isign, 1);
    swatch.stop();
    clov_deriv_time  += swatch.getTimeInSeconds();
