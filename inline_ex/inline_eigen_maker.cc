...
#include "inline_ex/inline_eigen_maker.h"
...
#include "actions_ex/eigenop_overlap.h"
...
// Factory Registration
  ...
  namespace InlineEigenMakerEnv
  {
    ...
    const std::string name = "EIGENMAKER";
    const std::string name2 = "EIGENERASER";
    ...
    bool registerAll()
    {
      ...
      if (!registered)
      {
        ...
        success &= TheInlineMeasurementFactory::Instance()
            .registerObject(name2, createMeasurement2);
        registered = true;
      }
      return success;
    }
...
// Parameter reading, writing, handling
  ...
  InlineEigenMakerParams::InlineEigenMakerParams(void)
  {
    frequency = 0;
  }
...
// The real work is done here
  ...
  void InlineEigenMaker::operator()( unsigned long update_no,
                                       XMLWriter& xml_out )
  {
  ...
  // Real work done here
  void InlineEigenMaker::func( unsigned long update_no,
                                 XMLWriter& xml_out)
  {
    START_CODE();
    ...
    QDPIO::cout << " EigenMaker: Make eigen system of " << params.param.fermact.id << " action." << std::endl;
    ...
    Handle<EigenOperator<LatticeFermion> > eigen;

    push(xml_out, "EigenMaker_measurements");
    
    if(TheNamedObjMap::Instance().check(params.named_obj.eigen_id)) 
    {
         QDPIO::cerr << params.named_obj.eigen_id << " has been assigned to some other object and can not be used here" << std::endl;
         QDP_abort(1);
    }
    else
    {
         TheNamedObjMap::Instance().create<Handle<EigenOperator<LatticeFermion> > >
                         (params.named_obj.eigen_id);
         TheNamedObjMap::Instance().getData<Handle<EigenOperator<LatticeFermion> > >
                         (params.named_obj.eigen_id)=eigen;
         
         if(params.param.fermact.id!="HWILSON"||params.param.fermact.id=="OVERLAP")
         {
             QDPIO::cerr << "Action " << params.param.fermact.id << " is not supported yet" << std::endl;
             QDP_abort(1);
         }
         ...
         if(params.param.fermact.id=="HWILSON")
             eigen=new HwilsonEigenOperator(params.param.fermact,U,params.param.Noeigen);
         if(params.param.fermact.id=="OVERLAP")
             eigen=new OverlapEigenOperator(params.param.fermact,U,params.param.Noeigen);

         EigenOperator<LatticeFermion> &es=*eigen;
         Complex value=es[0].val;

             	// Keep an array of all the xml output buffers
         if(params.param.file_exist==true)
         {
             //TODO:load the eigensystem;
         ...
         else
         {
             ...
             if(params.param.filename!="_NULL_")
                  eigen->save(params.param.filename);
         }
    }
    ...
    snoop.stop();
    QDPIO::cout << InlineEigenMakerEnv::name << ": total time = "
